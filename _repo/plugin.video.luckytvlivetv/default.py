...
def start():
	if username=="":
		...
		if auth == "":
			...
			line3 = ('[B][COLOR green]%s[/COLOR][/B]'%usern)
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        try: 
            ...
            mbs = '[B][COLOR green]%.02f MB of less than 5MB[/COLOR][/B]' % (currently_downloaded)
            e = '[B][COLOR green]Speed:  %.02f Mb/s ' % mbps_speed  + '[/COLOR][/B]'
...
def search():
	...
	text = searchdialog()
	if not text:
		xbmc.executebuiltin("XBMC.Notification([B][COLOR green][B]Search is Empty[/B][/COLOR][/B],Aborting search,4000,"+icon+")")
		return
	xbmc.log(str(text))
	open = tools.OPEN_URL(panel_api)
...
def accountinfo():
	try:
		...
		if not expiry=="":
			...
			tools.addDir('[B][COLOR green]Username :[/COLOR][/B] '+username,'','',icon,fanart,'')
			tools.addDir('[B][COLOR green]Password :[/COLOR][/B] '+password,'','',icon,fanart,'')
			tools.addDir('[B][COLOR green]Expiry Date:[/COLOR][/B] '+expiry,'','',icon,fanart,'')
			tools.addDir('[B][COLOR green]Account Status :[/COLOR][/B] %s'%status,'','',icon,fanart,'')
			tools.addDir('[B][COLOR green]Current Connections:[/COLOR][/B] '+ active,'','',icon,fanart,'')
			tools.addDir('[B][COLOR green]Allowed Connections:[/COLOR][/B] '+connects,'','',icon,fanart,'')
			tools.addDir('[B][COLOR green]Local IP Address:[/COLOR][/B] '+ip,'','',icon,fanart,'')
			tools.addDir('[B][COLOR green]External IP Address:[/COLOR][/B] '+extip,'','',icon,fanart,'')
			tools.addDir('[B][COLOR green]Kodi Version:[/COLOR][/B] '+str(KODIV),'','',icon,fanart,'')
	except:
		pass
...
if mode==None or url==None or len(url)<1:
	...
	xbmcgui.Dialog().ok('[B][COLOR green]LuckyTv[/COLOR][/B]','This Category Will Be Available Soon!')
	livecategory('url')
